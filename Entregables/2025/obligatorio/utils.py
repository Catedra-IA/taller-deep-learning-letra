...
def model_classification_report(model, dataloader, device, nclasses, digits=2, do_confusion_matrix=False, do_balanced_accuracy=False):
    ...
    report = classification_report(
        all_labels, all_preds, target_names=[str(i) for i in range(nclasses)], digits=digits
    )
...
def show_tensor_image_with_mask_overlay(image_tensor, mask_tensor, title="Image with Mask Overlay", figsize=(8, 8), alpha=0.5, mask_color='Reds'):
    ...
    Args:
        ...
        mask_color (str, optional): Color de la máscara. Por defecto es 'red'.
    """
